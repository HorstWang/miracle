...
run_db_container()
{
  ...
  docker run --name $CONTNR_NAME -d $IMG_NAME:$TAG
}
...
run_db_container_from_hub()
{
  ...
  docker run --name $CONTNR_NAME -d $REPO/$IMG_NAME:$TAG
}
...
#  TAG=$3
#
#  DB_CONTNR_ID=$(docker ps -a | grep $DB_CONTNR_NAME | awk '{ print $1 }')
#  docker commit $DB_CONTNR_ID $DB_IMG_NAME:$TAG
#}
...
# echo "export DB_CONTAINER_NAME=$DB_CONTNR_NAME" > ./docker-django/web_env
  cd ./docker-django/
  cp -r ../../kattegat/ ./
  docker build -t $WEB_IMG_NAME:$TAG .
  rm -rf ./kattegat/
  cd ..
}
...
# docker run --name $WEB_CONTNR_NAME --link $DB_CONTNR_NAME:$DB_IMG_NAME -e DB_CONTAINER_NAME=$DB_CONTNR_NAME -e WEB_PORT=$WEB_P -p $WEB_P:$WEB_P -d $WEB_IMG_NAME:latest
  docker run --name $WEB_CONTNR_NAME --link $DB_CONTNR_NAME:$DB_CONTNR_NAME -e DB_CONTAINER_NAME=$DB_CONTNR_NAME -e WEB_PORT=$WEB_P -p $WEB_P:$WEB_P -d $WEB_IMG_NAME:$TAG
}
...
# docker run --name $WEB_CONTNR_NAME --link $DB_CONTNR_NAME:$DB_IMG_NAME -e DB_CONTAINER_NAME=$DB_CONTNR_NAME -e WEB_PORT=$WEB_P -p $WEB_P:$WEB_P -d $WEB_IMG_NAME:latest
  docker run --name $WEB_CONTNR_NAME --link $DB_CONTNR_NAME:$DB_CONTNR_NAME -e DB_CONTAINER_NAME=$DB_CONTNR_NAME -e WEB_PORT=$WEB_P -p $WEB_P:$WEB_P -d $REPO/$WEB_IMG_NAME:$TAG
}
...
init_db()
{
  ...
  WEB_APP_CONTAINER_ID=$(docker ps -a | grep $WEB_IMG_NAME\_$WEB_P | grep -v /$WEB_IMG_NAME | awk '{ print $1 }')
  docker exec $WEB_APP_CONTAINER_ID /bin/bash -c "source ./venv_py_3.6.2/bin/activate && cd ./kattegat/vplex/err_code_xml/ && python ./err_code_xml_parser.py -f VPlexErrCode.bin.Debug.50.xml -v 5.0 && python ./err_code_xml_parser.py -f VPlexErrCode.bin.Debug.51.xml -v 5.1 && python ./err_code_xml_parser.py -f VPlexErrCode.bin.Debug.52.xml -v 5.2 && python ./err_code_xml_parser.py -f VPlexErrCode.bin.Debug.53.xml -v 5.3 && python ./err_code_xml_parser.py -f VPlexErrCode.bin.Debug.54.xml -v 5.4 && python ./err_code_xml_parser.py -f VPlexErrCode.bin.Debug.55.xml -v 5.5 && python ./err_code_xml_parser.py -f VPlexErrCode.bin.Debug.60.xml -v 6.0 && deactivate && cd ~"
}
...
commit_container()
{
  ...
  APP_CONTAINER_ID=$(docker ps -a | grep $CONTNR_NAME | awk '{ print $1 }')
  echo "Commiting container $APP_CONTAINER_ID as $IMG_NAME:$TAG..."
  docker commit $APP_CONTAINER_ID $IMG_NAME:$TAG
}
...
push_image()
{
  ...
  IMG_ID=$(docker images -a | grep $IMG_NAME | grep $TAG | grep -v /$IMG_NAME | awk '{ print $3 }')
  ...
  then
    ...
    docker tag $IMG_ID $REPO/$IMG_NAME:$TAG
...
then
  for image in 'vplex_log_db' 'vplex_log_parser' 'vplex_log_server';do docker pull activemauney/$image:$IMAGE_TAG; done
elif [ "$ACTION" == 'push' ]
then
  docker_hub_login $REPO_NAME $REPO_PASSWORD
...
then
  ./purge_all_containers_jenkins.sh # 2>&1 >/dev/null
  ./purge_all_images_jenkins.sh # 2>&1 >/dev/null
elif [ "$ACTION" == 'run' ]
then
  docker_hub_login
